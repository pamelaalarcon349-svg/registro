      ...
      //  if(solicitud.Email !== tempEmail)
       {
        ...
        "<button type='button' class='btn btn-outline-success' onclick='verifyData("+JSON.stringify(data)+")'><i class='fa-solid fa-floppy-disk'></i></button>"+
...
function verifyData(data) {
    ...
    $.ajax(settings).done(function (response) {
        ...
        if(response.data > 0){
            //saveData(data);
            console.log(data);
            console.log('no se registro correo y usuario asociados a un anterior o actual proceso de admision');
        }
        else{
            saveData(data);
            console.log()
